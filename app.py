...
# ==============================================================================
# 6. ENGINE DI GENERAZIONE (IL PEZZO CHE MANCAVA)
# ==============================================================================

# Questo blocco deve stare FUORI dalla sidebar, allineato a sinistra.
if btn_gen:
    if not api_key_input: 
        st.error("❌ ERRORE: Chiave API mancante! Inseriscila in alto o nei secrets.")
    else:
        # Feedback visivo immediato
        st.toast("Analisi parametri avviata...", icon="⚙️")
        
        with st.spinner("CALCOLO VETTORIALE SOMATOTIPO & AI GENERATION..."):
            
            # 1. Calcoli Biometrici preliminari
            ...
            # 2. Preparazione Pacchetto Dati per l'AI
            dati_totali = { 
            ...
            if "errore" not in res_ai:
                # 4. Successo: Salvataggio in Sessione
                # Iniettiamo i dati biometrici nel JSON per non perderli
                res_ai['meta_biometria'] = {
                    'somato': somato_str, 'bf': bf_val, 'ffmi': ffmi_val, 'whr': whr_calc
                }
                ...
                # Queste variabili servono per i widget immediati
                ...
                # 5. VISUALIZZAZIONE SCHEDA (OUTPUT A SCHERMO)
                ...
                # Visualizzazione Esercizi con Immagini
                for day, ex_list in res_ai.get('tabella', {}).items():
                    with st.expander(f"🔴 {day.upper()}", expanded=True):
                        ...
                        for ex in lista:
                            ...
                            # Layout Colonna Img + Colonna Testo
