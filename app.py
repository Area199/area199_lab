...
def stima_durata_sessione(lista):
    ...
    for ex in lista:
        if isinstance(ex, dict):
            if any(x in ex.get('Esercizio','').lower() for x in ["cardio", "run", "bike"]):
                sec += 900
            else:
                try:
...
def genera_protocollo_petruzzi(dati_input, api_key):
    ...
    DATI: {dati_input['meta']} | OBIETTIVO: {dati_input['goal']} | LIMITI: {dati_input['limitazioni']}
    ISTRUZIONI: {dati_input['custom_instructions']}
    LOGICA:
    1. 3gg -> PPL o Full Body. 4gg -> Upper/Lower.
    2. CARDIO: OBBLIGATORIO IN %FTP E ZONE Z1/Z2 (Es. "20' Z2 @ 65% FTP").
    OUTPUT JSON RIGIDO:
    {{
        "mesociclo": "NOME FASE", "analisi_clinica": "ANALISI DIRETTA...", "warning_tecnico": "ORDINE SECCO...", "cardio_protocol": "Protocollo...",
