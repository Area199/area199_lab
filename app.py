...
def crea_report_totale(nome, dati_ai, grafici_html_list, df_img, limitazioni, bf, somatotipo, whr, ffmi, eta=30):
    ...
    # --- FIX CRASH: FORZATURA STRINGA ---
    nome_display = str(nome).upper() if nome else "ATLETA"
    # ------------------------------------
    ...
    for day, ex_list in dati_ai.get('tabella', {}).items():
        ...
        for ex in lista:
            ...
            img1, img2 = trova_img(nome_ex, df_img) # Usa la funzione trova_img corretta
    ...
    <div class="header"><h1>AREA 199 LAB</h1><p style="color:#888; font-size:10px;">ATLETA: {nome_display} | DATA: {oggi}</p></div>
...
if btn_gen:
    ...
    if not api_key_input:
        st.error("❌ ERRORE: Manca la API Key (inseriscila a sinistra).")
    elif not nome:
        st.error("❌ ERRORE: Manca il Nome dell'atleta.")
    else:
        # 2. ESECUZIONE
        with st.spinner("⚙️ ANALISI BIO-VETTORIALE IN CORSO..."):
            try:
                # Preparazione Dati
                dati_totali = {
                ...
                # CHIAMATA AI (Questa è la parte che mancava!)
                ...
                if "errore" not in res_ai:
                    ...
                    st.success("✅ PROTOCOLLO PRONTO! Scorri in basso.")
                else:
...
# ==============================================================================
# 7. EXPORT & SYNC (ANTEPRIMA + SALVATAGGIO NO-DRIVE)
# ==============================================================================

if 'last_ai' in st.session_state:
    st.markdown("---")
    st.header("📄 ANTEPRIMA & EXPORT")
    ...
    # 3. VISUALIZZAZIONE DIRETTA (QUESTO MANCAVA!)
    st.markdown("### 👁️ ANTEPRIMA SCHEDA")
    st.components.v1.html(html_report, height=700, scrolling=True)
    st.markdown("---")

    # 4. BOTTONI DI SALVATAGGIO
    col_save_1, col_save_2 = st.columns(2)
    
    # Bottone 1: Scarica HTML
    with col_save_1:
        st.download_button(
            label="📥 SCARICA HTML",
            data=html_report,
    ...
    # Bottone 2: Salva su Database
    with col_save_2:
        if st.button("💾 SALVA SU DATABASE (Invia a Cliente)", use_container_width=True):
            mail_sicura = st.session_state.get('last_email_sicura')
            if not mail_sicura:
                st.error("⚠️ Inserisci l'Email del cliente nel menu laterale!")
            else:
                with st.spinner("Salvataggio..."):
                    ...
                    if ok:
                        st.success(f"✅ Scheda inviata a: {mail_sicura}")
                        st.balloons()
                    else:
